...
SafeMalesOnly <- subset(combined.codas.all.sites, site=="SA")

# Create a vector that has the names of males that were not recorded during playbacks
NoPlaybacks <- c("SCA", "SCB", "SFA", 
                 "SME", "SMF", "SMG", "SMH", "SMI")

# Create a new column indicating if male was recorded spontaneously or during playbacks
SafeMalesOnly$Playback <- as.factor(ifelse(
  (SafeMalesOnly$individual %in% NoPlaybacks),
  0,  # if condition is met, put 0
  1   # else put 1
))

# Convert new column to a factor
SafeMalesOnly$Playback <- as.factor(SafeMalesOnly$Playback)

# Supervised clustering using spectrogram features by playback
...
features.unique.calls <- unique(combined.codas.all.sites$call.id)

CallstoRemove <- c('ICGG1_36.1',mfcc.unique.calls[mfcc.unique.calls %nin% features.unique.calls],
...
(unique(combined.codas.all.sites$site))
Site.description <- subset(combined.codas.all.sites,site=="SA" )
length(unique((Site.description$individual)))
nrow(Site.description)
min(table(Site.description$individual))
max((table(Site.description$individual)))
...
ml.model.specfeatures.svm.site <- e1071::svm(combined.codas.all.sites[,c(3:22)], 
                                ...
                                cross = nrow(combined.codas.all.sites))
...
ml.model.mfccs.svm$tot.accuracy 
...
MaleCoda.umap <- 
  umap::umap(all.mfcc.combined[,-c(1,2,180)],
             labels=as.factor(all.mfcc.combined$group),
             controlscale=TRUE,scale=3)

recorder.id.df.umap <-
  cbind.data.frame(
    MaleCoda.umap$layout[,1:2],
    as.factor(all.mfcc.combined$group)
  )
...
## Note 1 duration
note1dur.site <- cbind.data.frame(icc.site$note1dur, rep("Between-site",length(icc.site$note1dur)))
note1dur.group <- cbind.data.frame(icc.group$note1dur,rep("Between-male",length(icc.group$note1dur)))
note1dur.obs <- cbind.data.frame(icc.obs$note1dur,rep("Within-male",length(icc.obs$note1dur)))
...
## Note 2 duration
note2dur.site <- cbind.data.frame(icc.site$note2dur, rep("Between-site",length(icc.site$note2dur)))
note2dur.group <- cbind.data.frame(icc.group$note2dur,rep("Between-male",length(icc.group$note2dur)))
note2dur.obs <- cbind.data.frame(icc.obs$note2dur,rep("Within-male",length(icc.group$note2dur)))
...
## Call duration
call.dur.site <- cbind.data.frame(icc.site$call.dur, rep("Between-site",length(icc.site$call.dur)))
call.dur.group <- cbind.data.frame(icc.group$call.dur,rep("Between-male",length(icc.group$call.dur)))
call.dur.obs <- cbind.data.frame(icc.obs$call.dur,rep("Within-male",length(icc.group$call.dur)))
...
## Note rate
noterate.site <- cbind.data.frame(icc.site$noterate, rep("Between-site",length(icc.site$noterate)))
noterate.group <- cbind.data.frame(icc.group$noterate,rep("Between-male",length(icc.group$noterate)))
noterate.obs <- cbind.data.frame(icc.obs$noterate,rep("Within-male",length(icc.obs$noterate)))
